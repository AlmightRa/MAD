...
class RmWrapper ( DbWrapperBase ) :
    ...
    def get_next_raid_hatches ( self , delay_after_hatch , geofence_helper=None ) :
        ...
        for (start , latitude , longitude) in res :
            if latitude is None or longitude is None :
                ...
                logger.debug ( "Excluded hatch at {}, {} since the coordinate is not inside the given include fences" ,
                               str (
    ...
    def submit_fort_proto (self, origin, map_proto, mapper):
        logger.debug ( "RmWrapper::submit_fort called" )

        now = datetime.utcfromtimestamp ( time.time () ).strftime ( "%Y-%m-%d %H:%M:%S" )

        ## update gym info
        gym_status_and_defenders = map_proto.get( "gym_status_and_defenders" )
        if (gym_status_and_defenders is None) :
            return

        pokemon_fort_proto = gym_status_and_defenders.get ( "pokemon_fort_proto" )
        if (pokemon_fort_proto is None) :
            return

        gym_details = pokemon_fort_proto.get ( "gym_details" )
        if (gym_details is None) :
            return

        guard_pokemon_display = gym_details.get ( "guard_pokemon_display" )
        if (guard_pokemon_display is None) :
            return

        gym_display = gym_details.get ( "gym_display" )
        if (gym_display is None) :
            return

        defenders_list = gym_status_and_defenders.get ( "gym_defender" )
        gym_defenders = {}

        i = 0
        for gd in defenders_list:
            gym_defenders[i] = gd
            i += 1

        # get values
        gym_id = pokemon_fort_proto.get ( "id" )
        owned_by_team = gym_details.get ( "owned_by_team" )
        guard_pokemon = gym_details.get ( "guard_pokemon" )
        slots_available = gym_details.get ( "slots_available" )
        enabled = gym_details.get ( "enabled", True )
        total_gym_cp = gym_display.get ( "total_gym_cp" )
        is_in_battle = gym_details.get ( "is_in_battle" )
        gender_value = guard_pokemon_display.get ( "gender_value" )
        form_value = guard_pokemon_display.get ( "form_value" )
        costume_value = guard_pokemon_display.get ( "costume_value" )
        weather_boosted_value = gym_details.get ( "weather_boosted_value" )
        is_shiny = guard_pokemon_display.get ( "is_shiny" )
        banner_url = pokemon_fort_proto.get ( "banner_url" )
        last_modified_ts = pokemon_fort_proto.get ( "last_modified_timestamp_ms" ) / 1000
        last_modified = datetime.utcfromtimestamp ( last_modified_ts ).strftime ( "%Y-%m-%d %H:%M:%S" )
        is_ex_raid_eligible = gym_details.get ( "is_ex_raid_eligible" )
        latitude = pokemon_fort_proto.get("latitude")
        longitude = pokemon_fort_proto.get ( "longitude" )

        # add columns to database
        # sponsor , sponsor_description, image_url , banner_url
        query = "REPLACE INTO gym" \
                "( gym_id , team_id , guard_pokemon_id , " \
                " slots_available , enabled , total_cp , " \
                " is_in_battle , gender , form ," \
                " costume , weather_boosted_condition , shiny ," \
                " last_modified , last_scanned , is_ex_raid_eligible ," \
                " banner_url, latitude , longitude )" \
                " VALUES " \
                "( %s , %s , %s ," \
                "%s , %s , %s ," \
                "%s , %s , %s ," \
                ...
                "%s , %s , %s ," \
                "%s , %s , %s )"
        vals1 = (gym_id , owned_by_team , guard_pokemon ,
                slots_available , enabled , total_gym_cp ,
                is_in_battle , gender_value , form_value ,
                costume_value , weather_boosted_value , is_shiny ,
                last_modified , now, is_ex_raid_eligible ,
                banner_url, latitude , longitude)
        res = self.execute ( query , vals1 , commit=True )

        # update gym details
        gym_name = map_proto.get ( "name" )
        description = map_proto.get ( "description" )
        url = map_proto.get ( "url" )

        query = "REPLACE INTO gymdetails " \
                "( gym_id , name , description , url , last_scanned )" \
                " VALUES " \
                "( %s , %s , %s , %s , %s )"
        vals2 = (gym_id , gym_name , description , url , now)
        res2 = self.execute ( query , vals2 , commit=True )

        # delete out gym pokemon
        # fk will delete out gympokemon
        query = "DELETE FROM gymmember WHERE gym_id = " + "'" + gym_id + "'"
        res3 = self.execute ( query , None , commit=True )

        arg_gymmember = []
        arg_gympokemon = []
        arg_trainer = []
        query_gymmember = "INSERT INTO gymmember " \
            "( gym_id , pokemon_uid , last_scanned , deployment_time , cp_decayed )" \
            " VALUES " \
            "( %s , %s , %s , %s , %s )"
        query_gympokemon = "INSERT INTO gympokemon " \
            "( pokemon_uid , last_seen , pokemon_id , " \
            "cp , trainer_name , num_upgrades , " \
            "move_1 , move_2 , height , " \
            "weight , stamina , stamina_max , " \
            "cp_multiplier , additional_cp_multiplier , iv_defense , " \
            "iv_stamina , iv_attack , gender , " \
            "form , costume, weather_boosted_condition , " \
            "shiny , badge_level )" \
            " VALUES " \
            "( %s , %s , %s , " \
            "%s , %s , %s ," \
            ...
            "%s , %s " \
            ")"
        query_trainer = "REPLACE INTO trainer " \
            "( name , team , level , " \
            "last_seen , xp , battles_won ) " \
            " VALUES " \
            "( %s , %s , %s , " \
            " %s , %s , %s )"

        for gm_num, gym_member in gym_defenders.items() :
            motivated_pokemon = gym_member.get ( "motivated_pokemon" )
            pokemon = motivated_pokemon.get ( "pokemon" )
            display = pokemon.get ( "display" )
            deployment_totals = gym_member.get ( "deployment_totals" )
            trainer_public_profile = gym_member.get ( "trainer_public_profile" )

            trainer_name = trainer_public_profile.get ( "name", "Empty" )
            trainer_level = trainer_public_profile.get ( "level", 0 )
            trainer_xp = trainer_public_profile.get ( "experience", 0 )
            trainer_battles_won = trainer_public_profile.get ( "battles_won", 0 )
            trainer_gym_badge = trainer_public_profile.get ( "gym_badge_type", 0 )

            pokemon_id = pokemon.get ( "id", 0 )
            cp = motivated_pokemon.get ( "cp_when_deployed", 0 )
            cp_decayed = pokemon.get ( "cp", 0 )

            num_upgrades = 0
            move_1 = pokemon.get ( "move_1", 0 )
            move_2 = pokemon.get ( "move_2", 0 )
            height = pokemon.get ( "height", 0 )
            weight = pokemon.get ( "weight", 0 )
            stamina = pokemon.get ( "stamina", 0 )
            stamina_max = pokemon.get ( "stamina_max", 0 )
            cp_multiplier = pokemon.get ( "cp_multiplier", 0 )
            ...
            gender = display.get ( "gender_value", 0 )
            form = display.get ( "form_value", 0 )
            costume = display.get ( "costume_value", 0 )
            weather_boosted_condition = display.get ( "weather_boosted_value", 0 )
            shiny = display.get ( "is_shiny", 0 )

            pokemon_uid = str( uuid.uuid1 ())
            deploy_ms = motivated_pokemon.get ( "deploy_ms", 0) / 1000
            deployment_time = datetime.utcfromtimestamp ( deploy_ms ).strftime ( "%Y-%m-%d %H:%M:%S" )

            # update gym member args
            arg_gymmember.append((gym_id, pokemon_uid, now, deployment_time, cp_decayed))

            # update gym pokemon args
            arg_gympokemon.append ((pokemon_uid , now , pokemon_id ,
                                    cp , trainer_name , num_upgrades ,
                                    move_1 , move_2 , height ,
                                    weight , stamina , stamina_max ,
                                    cp_multiplier , additional_cp_multiplier , iv_defense ,
                                    iv_stamina , iv_attack , gender ,
                                    form , costume , weather_boosted_condition ,
                                    shiny , trainer_gym_badge))

            # update trainer
            arg_trainer.append((trainer_name , owned_by_team , trainer_level ,
                                now , trainer_xp , trainer_battles_won))

        # update gym related queries
        res4 = self.executemany(query_gymmember, arg_gymmember, commit=True)
        res5 = self.executemany(query_gympokemon, arg_gympokemon, commit=True)
        res6 = self.executemany(query_trainer, arg_trainer, commit=True)
        logger.debug(res4)
    ...
    def get_gym_infos ( self , id=False ) :
        ...
        for (gym_id , latitude , longitude , name , description , url , team_id) in res :
            gyminfo [ gym_id ] = self.__encode_hash_json ( team_id , float ( latitude ) , float ( longitude ) ,
    ...
    def update_insert_weather ( self , cell_id , gameplay_weather , capture_time , cloud_level=0 , rain_level=0 ,
                                wind_level=0 ,
    ...
    def get_to_be_encountered ( self , geofence_helper , min_time_left_seconds ,
        ...
        for latitude , longitude , encounter_id , spawnpoint_id , pokemon_id , expire in results :
            if pokemon_id not in eligible_mon_ids :
                ...
                logger.debug (
                    "Excluded encounter at {}, {} since the coordinate is not inside the given include fences" , str (
                        latitude ) , str ( longitude ) )
                continue
    ...
    def __encode_hash_json ( self , team_id , latitude , longitude , name , description , url ) :
        return (
            { 'team_id' : team_id , 'latitude' : latitude , 'longitude' : longitude , 'name' : name ,
              'description' : '' ,
              'url' : url })
    ...
    def __extract_args_single_pokestop ( self , stop_data ) :
        ...
        return stop_data [ 'id' ] , 1 , stop_data [ 'latitude' ] , stop_data [
            'longitude' ] , last_modified , lure , now
    ...
    def __extract_args_single_pokestop_details ( self , stop_data ) :
        ...
        return stop_data [ 'fort_id' ] , 1 , stop_data [ 'latitude' ] , stop_data [
            'longitude' ] , last_modified , now , name , image [ 0 ]
